...
\item The hyperparameters are left as default if not mentioned otherwise. More on those can be found on their GitHub\myfootnote{\url{https://github.com/cmusphinx/g2p-seq2seq}}. 
\end{itemize}
...
Table \ref{tab:res_long_full} shows the results for all models that I trained for 200,000 steps. It is intersting that the results for all three models are actually very similar. I was surprised by teh fact that sometimes the cleaned version did not improve the performance although the cleaning was supposed to make the data more consistent. An intersting result is the one for Zulu. The cleaned model performed a lot better than the other two. As this result seems rather strange, I will have a close look at the data again.
...
\tab{tab:res_long_full}{This table shows the results for setting 2 (BS), setting 4 (BS-clean) and setting 6 (F2)}{
...
cmn       & broad         & 17.6   & \textit{3.9}    & 17.0         & \textit{4.0}          & 18.1   & \textit{4.4}    \\
deu       & broad         & 37.1   & \textit{4.8}    & 38.1         & \textit{5.0}          & 38.6   & \textit{5.9}    \\
deu       & narrow        & 52.2   & \textit{7.1}    & 56.9         & \textit{8.4}          & 55.9   & \textit{9.4}    \\
ell       & broad         & 7.1    & \textit{0.6}    & 9.0          & \textit{0.7}          & 9.1    & \textit{0.8}    \\
eng us    & broad         & 50.7   & \textit{9.5}    & 51.2         & \textit{10.2}         & 51.3   & \textit{10.4}   \\
eng us    & narrow        & 84.6   & \textit{31.4}   & 84.2         & \textit{33.1}         & 84.9   & \textit{32.3}   \\
eng uk    & broad         & 45.5   & \textit{8.6}    & 47.5         & \textit{9.2}          & 47.6   & \textit{9.7}    \\
eng uk    & narrow        & 90.3   & \textit{30.2}   & 94.8         & \textit{35.3}         & 93.7   & \textit{34.2}   \\
eus       & broad         & 21.2   & \textit{2.7}    & 19.6         & \textit{2.3}          & 21.7   & \textit{3.2}    \\
fin       & broad         & 2.8    & \textit{0.2}    & 3.3          & \textit{0.3}          & 8.9    & \textit{3.1}    \\
fin       & narrow        & 3.2    & \textit{0.3}    & 3.9          & \textit{0.4}          & 9.0    & \textit{3.5}    \\
fra       & broad         & 5.3    & \textit{0.7}    & 5.8          & \textit{0.8}          & 5.9    & \textit{0.8}    \\
hin       & narrow        & 7.7    & \textit{1.2}    & 8.4          & \textit{1.4}          & 8.4    & \textit{1.3}    \\
hin       & broad         & 4.4    & \textit{0.7}    & 6.4          & \textit{1.0}          & 6.3    & \textit{1.0}    \\
ind       & broad         & 37.9   & \textit{5.3}    & 34.9         & \textit{5.3}          & 39.3   & \textit{6.1}    \\
ind       & narrow        & 43.1   & \textit{5.4}    & 43.0         & \textit{5.6}          & 43.1   & \textit{5.6}    \\
jpn       & narrow        & 6.5    & \textit{0.6}    & 6.8          & \textit{0.6}          & 6.6    & \textit{0.7}    \\
kat       & broad         & 0.0    & \textit{0.0}    & 0.0          & \textit{0.0}          & 1.0    & \textit{0.8}    \\
kor       & narrow        & 23.4   & \textit{4.1}    & 25.3         & \textit{4.4}          & 25.8   & \textit{4.6}    \\
mya       & broad         & 35.1   & \textit{6.5}    & 36.0         & \textit{6.9}          & 88.0   & \textit{17.4}   \\
rus       & narrow        & 1.9    & \textit{0.2}    & 2.4          & \textit{0.3}          & 5.0    & \textit{1.5}    \\
spa ca    & broad         & 1.1    & \textit{0.1}    & 1.3          & \textit{0.1}          & 2.2    & \textit{0.7}    \\
spa ca    & narrow        & 2.3    & \textit{0.3}    & 2.2          & \textit{0.3}          & 2.8    & \textit{0.6}    \\
spa la    & broad         & 1.4    & \textit{0.1}    & 1.5          & \textit{0.2}          & 1.9    & \textit{0.6}    \\
spa la    & narrow        & 2.6    & \textit{0.3}    & 2.7          & \textit{0.3}          & 2.7    & \textit{0.4}    \\
tgl       & broad         & 28.4   & \textit{4.6}    & 31.2         & \textit{5.2}          & 33.9   & \textit{5.0}    \\
tgl       & narrow        & 45.5   & \textit{6.4}    & 47.3         & \textit{7.0}          & 48.6   & \textit{6.9}    \\
tha       & broad         & 12.5   & \textit{2.6}    & 11.1         & \textit{2.5}          & 12.1   & \textit{3.1}    \\
tur       & broad         & 50.6   & \textit{7.8}    & 52.3         & \textit{7.5}          & 49.1   & \textit{7.2}    \\
tur       & narrow        & 55.1   & \textit{7.6}    & 55.6         & \textit{8.3}          & 54.8   & \textit{8.0}    \\
vie       & narrow        & 1.5    & \textit{0.8}    & 1.6          & \textit{0.8}          & 2.6    & \textit{1.5}    \\
zul       & broad         & 65.9   & \textit{10.7}   & 9.8          & \textit{0.9}          & 91.4   & \textit{12.0}   \\ \hline
\end{tabularx}
}{Results: long models}
