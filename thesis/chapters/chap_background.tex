...
& & fra (fre) & 7.5 \\
& & kor & 16.2 \\\cline{3-4}
& & \multicolumn{2}{c|}{low (1,000 pairs)} \\\cline{3-4}
& & ell (gre) & 20 \\
& & ady & 22 \\
& & isl (ice) & 10 \\
& & lav & 49 \\
...
& & cym (wel\_sw) & 10 \\
\hline
\multirow[t]{6}{0.14\textwidth}{SIG21: \citet{lo-nicolai-2021-linguistic} \\
\vspace{1cm}
\href{https://aclanthology.org/2021.sigmorphon-1.15/}{\underline{Link}}}& \multirow[t]{6}{0.5\textwidth}{Only their second model, UBC-2, outperforms the baseline. They analysed the errors of the baseline and extend it by adding penalties for wrong vowels and wrong diacritics. Errors on vowels actually decreased. The macro-average outperformed all other systems (low-resource task).
}& \multicolumn{2}{c|}{low (1,000 pairs)} \\\cline{3-4}
& & ady & 22 \\
& & khm & 28 \\
& & lav & 49 \\
...
& & slv & 47 \\
\hline
\multirow[t]{7}{0.14\textwidth}{SIG21: \citet{gautam.2021} \\
\vspace{1cm}
\href{https://aclanthology.org/2021.sigmorphon-1.16/}{\underline{Link}}}& \multirow[t]{7}{0.5\textwidth}{Dialpad-1: large majority-vote ensemble consisting of three different public models (weighted \ac{fst}, joint-sequence model trained with \ac{em} and a neural \ac{s2s}) two \ac{s2s} variants (LSTM and transformer) and two variations of the baseline. The \ac{wer} could have been reduced to around 25 if the  model had always chosen the gold reference when it was present in the predictions.
}& \multicolumn{2}{c|}{high (41,000 pairs)} \\\cline{3-4}
& & eng (eng\_us)  &  37.43 \\
& &   &   \\
& &   &   \\
& &   &   \\
...
\item[\textsc{Character}] An abstract representation of the shape of the grapheme. Can in theory not be represented visually, as this includes a font. 
\item[\textsc{Glyph}] The rendered and therefore visual representation of one character that can be identified by its code point. A glyph is rendered in a specific font in a specific context. No matter how different it looks to the user, for Unicode all different representations of one code point are exactly the same.
\end{description}
...
\begin{description}
\item[\textsc{Grapheme clusters}] A grapheme cluster is one visual letter that is represented as more than one code point. This is the case for diacritic marks. Note that sometimes, these can be precomposed and the combination of those two or more characters is assigned a new number. 
\end{description} 

Additional complexity is added through the possibility of Unicode to create Unicode locales. These allow users to specify language- or writing-system-specific cases. 
