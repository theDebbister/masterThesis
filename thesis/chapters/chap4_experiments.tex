...
\subparagraph{WikiPron} Some of the WikiPron data has already been filtered as has been explained in section \ref{data_qual}. However, as I am not only using their filtered data I will have to do some preprocessing as well and also make some additional changes also to the filtered versions. 
...
\textipa{"} 					& \scriptsize{COMBINING VERTICAL LINE ABOVE}				& NULL						& This is not the same code point as the MODIFIER LETTER LOW VERTICAL LINE but it is most probably meant to be that. It occurs very rarely in the datasets. \\\hline
$\sim$							& \scriptsize{SWUNG DASH} 									& NULL 						& \multirow[t]{3}{\hsize}{All of these are characters that occur less than 10 times in a dataset for one language. As these experiments were conducted on the high resource languages, all of the languages contain at least 1,000 samples.}  \\
,							 	& \scriptsize{COMMA} 										& NULL 						&  \\
\textipa{\~v} 					& \scriptsize{TILDE} 										& NULL 						&  \\\hline
$^x$						& \scriptsize{MODIFIER LETTER SMALL X} 						& NULL 						&  \review{add explanation} \\
\hline
( ) 						& \scriptsize{(SUPERSCRIPT) [LEFT | RIGHT] PARENTHESIS}		& NULL							& Parentheses are typically used to denote optionality. In Chinese the superscript parentheses are used to denote optional tones. Those might be typical cases for non-standard varieties (see \href{https://www.hackingchinese.com/optional-obligatory-tone-change-rules-mandarin/}{example}). As ambiguities cannot be handled, parentheses and what they contain will be excluded. 
